...
local mappings = {
	...
	E = {
		name = "Edit",
		t = { "<cmd>TSJToggle<cr>", "Toggle Join" },
	...
	J = {
		...
		u = { "<cmd>lua require('jdtls').update_projects_config()()<cr>", "Update Projects Config" },
	},
	K = { "<cmd>DevdocsOpenFloat<cr>", "DevDocs" },
	L = {
		name = "Leetcode",
		l = { "<cmd>Leet lang<cr>", "Language" },
		c = { "<cmd>Leet console<cr>", "Console Toggle" },
		q = { "<cmd>Leet tabs<cr>", "Open Questions" },
	...
	n = {
		...
		n = { "<cmd>Noice enable<cr>", "Noice enable" },
	...
	s = {
		...
		g = { "<cmd>Telescope live_grep<cr>", "Search Project (grep)" },
	...
	u = {
		...
		L = { "<cmd>Trouble symbols toggle focus=false<cr>", "Toggle Symbols" },
		s = { "<cmd>Trouble lsp toggle win.position=right<cr>", "Toggle LSP" },
		b = { "<cmd>Trouble diagnostics toggle filter.buf=0<cr>", "Toggle Buffer Diagnostics" },
		t = { "<cmd>Trouble diagnostics toggle <cr>", "Toggle Diagnostics" },
	},
	w = { "<cmd>:Telescope session-lens<cr>", "Switch Workspace" },
...
return {
	...
	config = function()
		...
		require("which-key").register(local_mappings, l_opts)
	end,
}
